...
│  └────────────┘ └────────────┘ └────────────┘      │
│                                                    │
│  ☰  Sidebar: Mission Deck, Export, Archive done    │
└────────────────────────────────────────────────────┘
```
...
- 允许多行富文本，当前以 TipTap HTML 字符串保存。
- 任务仍不拆分标题/描述；默认第一块是 `h3`，后续段落仍属于同一 content。
- 项目名也通过同一个 inline editor 编辑，默认第一块是 `h1`。
- 展示时可以限制最大高度或最大行数。
- 点击任务内容进入 inline 编辑。
- 失焦保存。
- 在标题块末尾按 Enter 插入正文段落。
- Esc 取消编辑。
...
interface Project {
	id: string;
	// TipTap HTML; plain text imports are normalized by the editor.
	name: string;
	order: number;
	archived: boolean;
